...
class StackedTimeFlowWidgetReceiver : AppWidgetProvider() {
    ...
    private fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
        ...
        views.setOnClickPendingIntent(R.id.widget_root_layout, pendingIntent)        // Use coroutines to fetch data in background
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // Sort timeFlows by remaining time percentage (lowest first)
                val sortedTimeFlows = timeFlows.sortedBy { item -> calculateTimeFlowPercentage(item) }
                
                // Limit to MAX_ITEMS_TO_SHOW
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (limitedTimeFlows.isEmpty()) {
                        // Show a message if no TimeFlows available
                        ...
                        // Add TimeFlow items to the container
                ...
                // Update widget with error message on main thread
      ...
      private fun createTimeFlowItemView(context: Context, timeFlow: TimeFlowItem): RemoteViews {
        ...
        // Format the date using TimeFlowItem's formatDateTime function
        ...
        itemView.setTextViewText(R.id.item_days_left, "$daysLeft ${if (daysLeft == 1) "day" else "days"} left")
      ...
      private fun calculateTimeFlowPercentage(timeFlow: TimeFlowItem): Float {
        // Use the built-in function from TimeFlowItem
    ...
    private fun calculateDaysLeft(timeFlow: TimeFlowItem): Int {
        ...
        return TimeUnit.MILLISECONDS.toDays(diff).toInt() + 1 // +1 to include the current day
    }
}
